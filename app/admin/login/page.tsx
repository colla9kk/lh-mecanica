...
export default function LoginPage() {
  ...
  return (
    <main className="grid min-h-screen bg-[#0e131b] p-5 text-white lg:grid-cols-2">
      <section className="hidden flex-col justify-between rounded-3xl bg-[#b91424] p-12 lg:flex">
        <Link href="/" className="flex items-center gap-3 font-black"><img src="/images/logo-lh.jpeg" alt="Logo Mecânica LH" width={108} height={72} className="rounded-xl bg-white object-contain" /> LH Mecânica</Link>
      ...
      <section className="grid place-items-center px-2 py-12 sm:px-8">
        <form onSubmit={login} className="w-full max-w-md">
          ...
          <span className="grid h-12 w-12 place-items-center rounded-xl bg-white/10 text-[#ff6674]"><LockKeyhole /></span>
          ...
          <button disabled={loading} className="mt-7 min-h-13 w-full rounded-xl bg-[#b91424] px-5 font-black disabled:opacity-60">{loading ? "Entrando..." : "Entrar"}</button>
        </form>
      </section>
    </main>
  );
