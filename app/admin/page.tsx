...
export default function AdminPage() {
  ...
  if (!ready) return <main className="grid min-h-screen place-items-center bg-[#0e131b] text-white"><RefreshCw className="animate-spin text-[#e8502a]" /></main>;
    ...
    <main className="min-h-screen bg-[#f3f4f4] text-[#131a22] lg:grid lg:grid-cols-[260px_1fr]">
      <aside className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-[#0e131b] p-5 text-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0 ${mobileMenu ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 font-black"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e8502a]"><Wrench size={20} /></span> LH Mecânica</div>
...
function NavButton({ active, icon: Icon, children, onClick }: { active: boolean; icon: typeof Search; children: React.ReactNode; onClick: () => void }) {
  return <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-bold transition ${active ? "bg-[#e8502a] text-white" : "text-white/58 hover:bg-white/6 hover:text-white"}`}><Icon size={18} />{children}</button>;
}

function PageTitle({ eyebrow, title, copy, action }: { eyebrow: string; title: string; copy?: string; action?: React.ReactNode }) {
  return <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[#e8502a]">{eyebrow}</p><h1 className="mt-2 text-3xl font-black tracking-[-.035em] sm:text-4xl">{title}</h1>{copy && <p className="mt-3 max-w-2xl text-[#68717a]">{copy}</p>}</div>{action}</div>;
}
...
function Dashboard({ customers, vehicles, orders, loading, onPlate, onNewOrder, onSelectOrder }: { customers: Customer[]; vehicles: Vehicle[]; orders: ServiceOrder[]; loading: boolean; onPlate: () => void; onNewOrder: () => void; onSelectOrder: (o: ServiceOrder) => void }) {
  ...
  return <><PageTitle eyebrow="Hoje na oficina" title="Visão geral" copy="Acompanhe os atendimentos e encontre rapidamente o que precisa." action={<div className="flex gap-3"><button onClick={onPlate} className="rounded-xl border border-black/10 bg-white px-4 py-3 text-sm font-black">Consultar placa</button><button onClick={onNewOrder} className="rounded-xl bg-[#e8502a] px-4 py-3 text-sm font-black text-white">Nova OS</button></div>} />
...
function CustomersPanel({ customers, onSaved, onError }: { customers: Customer[]; onSaved: () => void; onError: (e: string) => void }) {
  ...
  return <><PageTitle eyebrow="Relacionamento" title="Clientes" copy="Cadastre os proprietários antes de adicionar os veículos." action={<button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 rounded-xl bg-[#e8502a] px-5 py-3 text-sm font-black text-white"><Plus size={17} /> Novo cliente</button>} />
...
function VehiclesPanel({ vehicles, customers, onSaved, onError, onNewOrder }: { vehicles: Vehicle[]; customers: Customer[]; onSaved: () => void; onError: (e: string) => void; onNewOrder: (id: string) => void }) {
  ...
  return <><PageTitle eyebrow="Frota atendida" title="Veículos" copy="Cada placa mantém seu próprio histórico de ordens." action={<button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 rounded-xl bg-[#e8502a] px-5 py-3 text-sm font-black text-white"><Plus size={17} /> Novo veículo</button>} />
...
function OrdersPanel({ orders, vehicles, presetVehicleId, clearPreset, onSaved, onError, onSelectOrder }: { orders: ServiceOrder[]; vehicles: Vehicle[]; presetVehicleId: string; clearPreset: () => void; onSaved: () => void; onError: (e: string) => void; onSelectOrder: (o: ServiceOrder) => void }) {
  ...
  return <><PageTitle eyebrow="Atendimentos" title="Ordens de serviço" copy="Crie uma nova ordem a cada visita e preserve o histórico do veículo." action={<button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 rounded-xl bg-[#e8502a] px-5 py-3 text-sm font-black text-white"><Plus size={17} /> Nova OS</button>} />
    {showForm && <form onSubmit={submit} className="mb-8 rounded-2xl border border-black/8 bg-white p-5 sm:p-7"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Select label="Veículo" required value={form.vehicle_id} onChange={(v) => setForm({ ...form, vehicle_id: v })} options={vehicles.map((v) => [v.id, `${v.plate} • ${v.brand} ${v.model}`])} /><Input label="Data de entrada" type="date" required value={form.entry_date} onChange={(v) => setForm({ ...form, entry_date: v })} /><Input label="Previsão de entrega" type="date" value={form.expected_delivery_date} onChange={(v) => setForm({ ...form, expected_delivery_date: v })} /><Input label="Quilometragem" type="number" value={form.mileage} onChange={(v) => setForm({ ...form, mileage: v })} /></div>
      ...
      <div className="mt-7 flex items-center justify-between"><h2 className="font-black">Serviços e peças</h2><button type="button" onClick={() => setItems([...items, { ...emptyItem }])} className="text-sm font-black text-[#e8502a]">+ Adicionar item</button></div>
...
function OrderDetails({ order, onClose, onChanged, onError }: { order: ServiceOrder; onClose: () => void; onChanged: () => void; onError: (e: string) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] bg-black/55 p-3 backdrop-blur-sm sm:p-6"><div className="ml-auto h-full w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-start justify-between border-b border-black/8 bg-white p-5 sm:p-7"><div><p className="text-sm font-bold text-[#e8502a]">{order.order_number}</p><h2 className="mt-1 text-2xl font-black">{order.vehicle?.plate} • {order.vehicle?.brand} {order.vehicle?.model}</h2></div><button onClick={onClose} aria-label="Fechar"><X /></button></div>
