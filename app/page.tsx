...
import { ArrowRight, CarFront, CheckCircle2, Clock3, MapPin, Menu, Phone, ShieldCheck, Wrench, X } from "lucide-react";
    ...
    <main className="min-h-screen bg-[#f5f5f2] text-[#111820]">
      <header className="sticky top-0 z-50 border-b border-black/10 bg-[#f5f5f2]/95 backdrop-blur">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          <Link href="/" className="flex items-center gap-3 font-black tracking-tight">
            <span className="grid h-11 w-11 place-items-center rounded-xl bg-[#e8502a] text-white shadow-[0_8px_24px_rgba(232,80,42,.28)]"><Wrench size={23} /></span>
          ...
          <nav className="hidden items-center gap-8 text-sm font-semibold md:flex">
            <a href="#servicos" className="hover:text-[#e8502a]">Serviços</a><a href="#triagem" className="hover:text-[#e8502a]">Avaliação</a><a href="#contato" className="hover:text-[#e8502a]">Contato</a><Link href="/admin/login" className="text-[#6d747c] hover:text-[#111820]">Área da oficina</Link>
          </nav>
      ...
      <section className="relative overflow-hidden bg-[#0e131b] text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-[1.15fr_.85fr] lg:px-8 lg:py-28">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm text-white/80"><span className="h-2 w-2 rounded-full bg-[#e8502a]" /> Atendimento em Guarujá e região</span>
            <h1 className="mt-7 max-w-3xl text-5xl font-black leading-[.98] tracking-[-.045em] sm:text-6xl lg:text-7xl">Seu carro em boas mãos, <span className="text-[#ff6a3d]">sem enrolação.</span></h1>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#triagem" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#e8502a] px-6 font-bold hover:bg-[#ff6238]">Solicitar avaliação <ArrowRight size={18} /></a>
          ...
          <div className="grid content-end gap-4 sm:grid-cols-2">
            {[[Clock3, "Socorro 24h", "Atendimento quando você mais precisa"], [ShieldCheck, "Orçamento claro", "Serviços e valores registrados"], [CarFront, "Histórico do carro", "Cada atendimento fica organizado"], [CheckCircle2, "Serviço responsável", "Cuidado do diagnóstico à entrega"]].map(([Icon, title, copy]) => {
              const C = Icon as typeof Clock3;
              return <article key={String(title)} className="rounded-2xl border border-white/10 bg-white/[.055] p-5 backdrop-blur"><C className="text-[#ff6a3d]" /><h2 className="mt-5 font-bold">{String(title)}</h2><p className="mt-2 text-sm leading-6 text-white/58">{String(copy)}</p></article>;
            })}
          </div>
        </div>
      </section>
      ...
      <section id="triagem" className="bg-white py-20">
        <div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[.75fr_1.25fr] lg:px-8">
          <div><p className="text-sm font-black uppercase tracking-[.18em] text-[#e8502a]">Triagem rápida</p><h2 className="mt-4 text-4xl font-black tracking-[-.035em]">Explique o problema antes de trazer o carro.</h2><p className="mt-5 leading-7 text-[#69717a]">Preencha os dados e envie direto para o WhatsApp da oficina. Isso ajuda a agilizar o atendimento.</p></div>
          <form onSubmit={submitTriage} className="grid gap-5 rounded-3xl border border-black/8 bg-[#f7f7f4] p-6 shadow-[0_20px_70px_rgba(17,24,32,.08)] sm:grid-cols-2 sm:p-8">
            ...
            <button className="min-h-14 rounded-xl bg-[#e8502a] px-6 font-black text-white hover:bg-[#d84622] sm:col-span-2">Enviar pelo WhatsApp</button>
          </form>
        </div>
      </section>

      <footer id="contato" className="bg-[#0e131b] text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 md:grid-cols-3 lg:px-8"><div><h2 className="text-xl font-black">{business.name}</h2><p className="mt-3 text-sm text-white/55">{business.tagline}</p></div><div className="space-y-3 text-sm text-white/72"><p className="flex gap-2"><Phone size={17} /> {business.phone}</p><p className="flex gap-2"><MapPin size={17} /> {business.address}</p></div><div className="text-sm text-white/55 md:text-right"><p>{business.hours}</p><p className="mt-3">© {new Date().getFullYear()} {business.shortName}</p></div></div></footer>
    </main>
  );
}
