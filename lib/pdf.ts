...
export function downloadOrderPdf(order: ServiceOrder) {
  ...
  if (order.diagnosis) {
    ...
    doc.text(lines, 14, startY + 7); startY += lines.length * 5 + 14;
  }
  autoTable(doc, {
    startY,
    ...
    body: items.map((item) => [
      ...
      item.description, item.quantity.toLocaleString("pt-BR"), money(item.unit_price), money(item.subtotal),
    ]),
  ...
  if (signatureY < 283) {
    doc.setDrawColor(130); doc.line(18, signatureY, 88, signatureY); doc.line(122, signatureY, 192, signatureY);
    doc.setFontSize(9); doc.setFont("helvetica", "normal");
  ...
  doc.save(`${order.order_number}-${vehicle?.plate || "veiculo"}.pdf`);
}
